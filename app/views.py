...
from .faiss_retrieval.features import (
...
def image_upload_view(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"url": "/image-search-result/{}".format(query_image.id)}
            )
...
def image_search_result(request, pk):
    ...
    if request.method == "GET":
        try:
            ...
            query_features = extract_CLIP_image_features(retrieve_path)
            # Query the index for similar images
            _, indices = query_index(
                index, query_features
            )  # Retrieve 5 nearest neighbors
            ...
            for obj in retrieved_objects:
                ...
                results.append(
                    (result_url, obj.category, obj.gender)
                )  # Access model fields
            print(results)
            end_time = time.time()
...
def text_search_result(request):
    """Retrieves similar images based on a query text."""
    if request.method == "POST":
        try:
            start_time = time.time()
            ...
            query_features = extract_CLIP_text_features(query_text)
            # Query the index for similar images
            _, indices = query_index(
                index, query_features
            )  # Retrieve 5 nearest neighbors
            ...
            print("Thời gian tìm kiếm: {:.2f} giây".format(process_time))
            with open("LOG_SEARCH_TIME.txt", "a") as log:
                log.write(f"\n{query_text}, {process_time}, {top_5_indices}, {results}")
            return render(
                request,
                "app/home.html",
                context={
